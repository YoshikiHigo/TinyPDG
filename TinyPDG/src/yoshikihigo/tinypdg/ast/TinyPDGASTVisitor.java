...
public class TinyPDGASTVisitor extends NaiveASTFlattener {
	...
	public boolean visit(final SuperMethodInvocation node) {
		...
		if (this.inMethod) {
			...
			int complexity = 1; // メソッド呼び出しの複雑度を1とする
			int depth = 1;
	...
	public boolean visit(final ArrayInitializer node) {
		...
		if (this.inMethod) {
			int complexity = 1; // 配列初期化の複雑度は１にしておく
			int depth = 1;
			final StringBuilder text = new StringBuilder();
			text.append("{");
	...
	public boolean visit(final ClassInstanceCreation node) {
		...
		if (this.inMethod) {
			int complexity = 1; // インスタンス生成の複雑度を1にしておく
			int depth = 1;
			final StringBuilder text = new StringBuilder();
			text.append("new ");
			text.append(node.getType().toString());
	...
	public boolean visit(final ConditionalExpression node) {
		...
		if (this.inMethod) {
			int complexity = 1; // 三項演算子の複雑度を１にしておく
			int depth = 1;
			final StringBuilder text = new StringBuilder();
	...
	public boolean visit(final ConstructorInvocation node) {
		...
		if (this.inMethod) {
			int complexity = 1; // コンストラクタ呼び出しの複雑度を1にしておく
			int depth = 1;
			final StringBuilder text = new StringBuilder();
			text.append("(");
	...
	public boolean visit(final SuperConstructorInvocation node) {
		...
		if (this.inMethod) {
			int complexity = 1; // スーパークラスのコストラクタ呼び出しの複雑度を1にしておく
			int depth = 1;
	...
	public boolean visit(final VariableDeclarationExpression node) {
		...
		if (this.inMethod) {
			int complexity = 1; // 変数宣言式の複雑度を1にしておく
			int depth = 1;
	...
	public boolean visit(final VariableDeclarationStatement node) {
		...
		if (this.inMethod) {
			int complexity = 0; // 変数宣言文の複雑度を1にしておく
			int depth = 1;
